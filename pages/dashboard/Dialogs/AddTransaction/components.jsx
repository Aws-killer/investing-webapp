...
import { Loader2, Check, ChevronsUpDown, ArrowRightLeft, TrendingUp, RefreshCw } from "lucide-react";
...
export const AssetSearch = () => {
  ...
  return (
    <div className="space-y-1.5">
      <FieldLabel>Select {assetLabel}</FieldLabel>
      <Popover open={open} onOpenChange={setOpen} modal={true}>
        <PopoverTrigger asChild>
          <button
            className="w-full h-10 flex items-center justify-between bg-background border border-border text-[13px] rounded-[6px] px-3 hover:border-foreground/30 transition-colors"
          >
            {formData.assetName
              ? <span className="font-semibold text-foreground">{formData.assetName}</span>
            ...
            <CommandList className="max-h-[220px] p-1">
              ...
              <CommandGroup>
                {availableAssets.map((item) => {
                  const ticker = item.symbol || item.isin || "";
                  ...
                  return (
                    <CommandItem
                      ...
                      value={`${name} ${ticker}`}
                      onSelect={() => {
                        setFormData(prev => ({ ...prev, assetId: String(item.id), assetSymbol: ticker, assetName: name }));
                        setOpen(false);
                      }}
                    ...
                    >
                      ...
                      <div className="flex flex-col min-w-0">
                        <span className="font-bold text-[13px] text-foreground">{ticker}</span>
...
export const TransactionFooter = () => {
  ...
  const isDisabled = isSubmitting || !formData.assetId || !formData.quantity || !formData.price || isFractionalError || isOverSelling;
